...
1. **GPS distance flakiness.** Rides sometimes compute zero distance. Code: `mobile/src/ride/realSensorSource.ts` (`expo-location` v56 `watchPositionAsync`, `BestForNavigation`, drops fixes with accuracy > 30 m). Verify the v56 permission + watch flow on device; handle Precise-Location off gracefully (prompt the user); consider a brief cold-start grace window before enforcing the 30 m gate. Prime Motion & Fitness + Location with a clear "why" screen before requesting.
